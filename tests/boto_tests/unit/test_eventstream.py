...
def create_mock_raw_stream(*data: bytes):
    ...
    raw_stream.content.iter_chunks = generator
    return raw_stream
...
def test_event_stream_wrapper_close():
    raw_stream = mock.Mock()
    event_stream = AioEventStream(raw_stream, None, None, '')
    event_stream.close()
    raw_stream.close.assert_called_once_with()


async def test_event_stream_initial_response():
    raw_stream = create_mock_raw_stream(
...
async def test_event_stream_initial_response_wrong_type():
    raw_stream = create_mock_raw_stream(
        b"\x00\x00\x00+\x00\x00\x00\x0e4\x8b\xec{\x08event-id\x04\x00",
        b"\x00\xa0\x0c{'foo':'bar'}\xd3\x89\x02\x85",
    )
    ...
    with pytest.raises(NoInitialResponseError):
        await event_stream.get_initial_response()
