...
async def test_service_uses_bearer_auth(
    ...
    async with patched_session.create_client(
        ...
        with ClientHTTPStubber(client) as http_stubber:
            ...
            await client.mock_operation(MockOpParam="mock-op-param-value")
...
async def test_service_does_not_use_bearer_auth(
...
async def test_user_agent_has_bearer_service_env_vars_feature_id(
    monkeypatch, patched_session, env_vars
):
    for key, value in env_vars.items():
        monkeypatch.setenv(key, value)
...
async def test_user_agent_does_not_have_bearer_service_env_vars_feature_id(
    monkeypatch, patched_session, env_vars, config
):
    for key, value in env_vars.items():
        monkeypatch.setenv(key, value)
